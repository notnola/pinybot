...
PLAYLIST_ALL_ITEMS_URL = 'https://www.googleapis.com/youtube/v3/playlistItems?' \
                         'key={0}&playlistId={1}&maxResults=50&part=status,contentDetails&pageToken={2}'
...
def playlist_all_videos(playlist_id):
    """
    Finds all the videos for a given playlist ID.

    The list returned will contain all the videos found in the playlist with the page token.
    :param playlist_id: str the playlist ID.
    :return: list[dict{'type=youTube', 'video_id', 'video_title', 'video_time'}] or None on failure.
    """
    all_videos_list = []
    _loop_video_results = True
    _page_token = ''
    url = PLAYLIST_ALL_ITEMS_URL.format(API_KEY, playlist_id, _page_token)

    while _loop_video_results:
        response = util.web.http_get(url=url, json=True, referer='http://tinychat.com')

        if response['json'] is not None:
            if 'nextPageToken' in response['json']:
                print('Next Page Token is found.')
                _page_token = str(response['json']['nextPageToken'])
            else:
                print('No Page Token found.')
                _page_token = ''
                _loop_video_results = False
