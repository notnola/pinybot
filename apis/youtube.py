...
# Quote-plus used to URL encode search terms.
from urllib import quote_plus

# YouTube API key.
YOUTUBE_API_KEY = 'AIzaSyCPQe4gGZuyVQ78zdqf9O5iEyfVLPaRwZg'

# TODO: No handlers could be found for logger "api.youtube"
...
def youtube_search(search):
    ...
    if search:
        ...
        youtube_search_url = 'https://www.googleapis.com/youtube/v3/search?' \
                             'type=video&key=%s' \
                             '&maxResults=50&q=%s&part=snippet' % (YOUTUBE_API_KEY, quote_plus(search))
...
def youtube_search_list(search, results=10):
    ...
    if search:
        youtube_search_url = 'https://www.googleapis.com/youtube/v3/search?type=video' \
                             '&key=%s&maxResults=50&q=%s&part=snippet' % (YOUTUBE_API_KEY, quote_plus(search))
...
def youtube_playlist_videos(playlist_id):
    ...
    playlist_details_url = 'https://www.googleapis.com/youtube/v3/playlistItems?' \
                           'key=%s&playlistId=%s' \
                           '&maxResults=50&part=status,contentDetails' % (YOUTUBE_API_KEY, playlist_id)
    ...
    while start_token:
        ...
        if api_response is not None:
            if 'nextPageToken' in api_response['content']:
                page_token = str(api_response['content']['nextPageToken'])
            else:
                page_token = ''
                start_token = False
            ...
            playlist_details_url = 'https://www.googleapis.com/youtube/v3/playlistItems?' \
                                   ...
                                   (YOUTUBE_API_KEY, playlist_id, page_token)
...
def youtube_time(video_id, check=False):
    ...
    youtube_details_url = 'https://www.googleapis.com/youtube/v3/videos?id=%s&key=%s&part=contentDetails,snippet' % \
    ...
    if api_response is not None:
        try:
            # TODO: String indices must be integer.
