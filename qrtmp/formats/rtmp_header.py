...
class RtmpHeaderHandler:
    ...
    def __init__(self, rtmp_stream):
        ...
        # The time in milliseconds (rounded to 3 decimal places) when the last header timestamp was encoded.
        self._last_encoded_timestamp = None
    ...
    def _get_header_mask(latest_full_header, header_to_encode):
        ...
        if latest_full_header.chunk_stream_id == header_to_encode.chunk_stream_id:
            ...
            # then it is an RTMP message in chunks.
            # Return that size corresponds to a type 3 header.
            # TODO: If both headers match we decide to send a continuation message, though continuation should only
            #       be used when we are writing the a continuation of the same data into the stream.
    ...
    def encode_into_stream(self, encode_header):
        ...
        print('chunk stream id chosen:', chunk_stream_id)
        ...
        if str(chunk_stream_id) in self._merged_chunk_streams_header:
            ...
            mask = self._get_header_mask(latest_full_header, encode_header)
        ...
        # timestamp delta since they are not present in this type of message.
        if mask == 0xc0:
            # TODO: Added chunk type information.
            # Set header chunk type to Type 3 (0xc0).
            encode_header.chunk_type = types.HEADER_TYPE_3_CONTINUATION
        else:
            # TODO: We Should get the timestamp from the previous header on the same chunk stream.

            # This applies to all header which is Type 2 (SAME_LENGTH_AND_STREAM) (0x80) or smaller.
            # Write the timestamp, if it fits, elsewhere state we need an extended timestamp and write it later.
            if mask == 0x80:
                print('Started to write header type 2')
                # We only need to write the timestamp delta for this type of header.

                # Set up a timestamp delta if it required.
                # if encode_header.timestamp is -1:
                # Use the last header on this chunk stream to work out the the timestamp delta.
            ...
            # Write message length, followed by the message type id.
                ...
                print('Started to write header type 1')
                # We will need to write the timestamp delta, body length and data type
                # for this type of header.
            ...
            # Write the stream id.
                ...
                print('Started to write header type 0')
                # We will need to write an absolute timestamp, body length, data type
                # and stream id for this type of packet.
