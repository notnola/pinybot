...
# Repository homepage: https://oddballz.github.io/pinybot/,
# Repository: https://github.com/oddballz/pinybot

# Core imports
import os
import sys
# import logging
import tinychat  # pinylib
import update
...
repo_info = '(https://github.com/oddballz/pinybot/ )'
authors = 'Cabbage Coding: *Nortxort* (https://github.com/nortxort/ ), *TechWhizZ199* (https://github.com/TechWhizZ199/ ),' + \
          '*Aida* (https://github.com/Autotonic/ ) & *NotNola* (https://github.com/NotNola/ )'

# TODO: Implement supplementary build names.
__version__ = '1.4.0'
# build_name =
...
# CONFIG = tinychat.fh.configuration_loader(str(os.path.join(os.path.dirname(__file__))) + '/' + CONFIG_FILE_NAME)
# os.path.dirname(os.path.abspath(__file__))
CURRENT_PATH = sys.path[0]
CONFIG_PATH = CURRENT_PATH + CONFIG_FILE_NAME
CONFIG = tinychat.fh.configuration_loader(CONFIG_PATH) # pinylib
...
# Initialise bot configurations:
# Set full path to files; sometimes essential when running on vps/web-server.
if CONFIG['webserver']:
    CONFIG['path'] = str(CURRENT_PATH) + '/' + CONFIG['path']

# Loads the 'ascii.txt' file with ASCII text into a dictionary
if CONFIG['ascii_chars']:
    ascii_dict = tinychat.fh.ascii_loader(CONFIG['path'] + CONFIG['ascii_file'])
...
special_unicode = {  # Assigned to:
    ...
    'no_width': u"\u200B",  # - applying bold styling to letter words which would otherwise not be parsed accurately;
...
def eightball():
    ...
    answers = ['It is certain.', 'It is decidedly so.', 'without a doubt', 'Yes - definitely.',
               ...
               'Yes, in due time.', 'My sources say no.', 'Definately not.', 'You will have to wait.',
               'I have my doubts.',
...
class TinychatBot(tinychat.TinychatRTMPClient):  # pinylib
    """ Overrides event methods in TinychatRTMPClient that the client should to react to. """

    # Initial settings
    init_time = tinychat.time.time()  # pinylib
    key = CONFIG['key']
    ...
    botteraccounts = tinychat.fh.file_reader(CONFIG['path'], CONFIG['botteraccounts'])  # pinylib
    ...
    autoforgive = tinychat.fh.file_reader(CONFIG['path'], CONFIG['autoforgive'])  # pinylib
    ...
    # Add-ons required/API deprecated:
    # cleverbot_client = None
    # akin_session = [False, None]
    # akin_answer = None
    # Vote-kick settings:
    # polling = False
    # pollers = {}
    ...
    def on_join(self, join_info_dict):
        ...
        if join_info_dict['account']:
            tc_info = tinychat.tinychat_api.tinychat_user_info(join_info_dict['account'])  # pinylib
            ...
            if join_info_dict['own']:
                ...
                self.console_write(tinychat.COLOR['bright_yellow'], join_info_dict['nick'] + ':' + str(join_info_dict['id']) +
                                   ' Has account: ' + join_info_dict['account'])  # pinylib

                badaccounts = tinychat.fh.file_reader(CONFIG['path'], CONFIG['badaccounts']) # pinylib
            ...
            if join_info_dict['id'] is not self.client_id:
                if self.no_guests:
                    ...
                    self.console_write(tinychat.COLOR['bright_cyan'], # pinylib
    ...
    # TODO: Do we still need to use this procedure?
    # -----------------------------------------------------
    # def camspam_prevention(self, user):
        # t = int(tinychat.time.time())
        ...
        # if user.camspam[0] is 2:
        #     if t - user.camspam[1] < 15:
        #         self.send_ban_msg(user.nick, user.id)

        # elif user.camspam[0] is 3:
        #     if t - user.camspam[1] < 30:
        #         self.send_ban_msg(user.nick, user.id)
        #     user.camspam[0] = 1
        #     user.camspam[1] = t
    # -----------------------------------------------------
            ...
            if not user.is_owner or not user.is_mod or not user.has_power:
                ...
                if len(uid_parts) is 2:
                    ...
                    if user_device == 'android':
                        self.console_write(tinychat.COLOR['cyan'], name + ':' + str(clean_uid)
                                            + ' is broadcasting from an android device.')  # pinylib
                    elif user_device == 'ios':
                        self.console_write(tinychat.COLOR['cyan'], name + ':' + str(clean_uid)
                                            + ' is broadcasting from an ios device.')  # pinylib
                    else:
                        self.console_write(tinychat.COLOR['cyan'], name + ':' + str(clean_uid)
                                            + ' is broadcasting from an unknown device.')  # pinylib

                # Assume this will not be needed anymore.
                # if CONFIG['camspam_prevention']:
                #    self.camspam_prevention(user)
    ...
    def on_nick(self, old, new, uid):
        # self.console_write(tinychat.COLOR['cyan'], 'Received client nick: \'' + str(self.client_nick) +
        ...
        if uid is not self.client_id:
            ...
            # TODO: Place back into code when add-on has been integrated.
            # Check if the user was being vote-kicked against
            # if self.polling:
            #    # if old == self.criminal:
            #    # TODO: Change criminal variable to something more decent
            #    if old == self.criminal:
            #        self.criminal = new
            #        self.send_bot_msg(special_unicode['indicate'] + ' The criminal changed their nick name: *' +
            #                          old + ' --> ' + new + '*', self.is_client_mod)

            if old.startswith('guest-'):
                bn = tinychat.fh.file_reader(CONFIG['path'], CONFIG['badnicks'])  # pinylib
            ...
            self.console_write(tinychat.COLOR['bright_cyan'], old + ':' + str(uid) + ' changed nick to: ' + new)  # pinylib
    ...
    def on_kick(self, uid, name):
        if uid is not self.client_id:
            ...
            if user.user_account:
                ...
                self.console_write(tinychat.COLOR['bright_red'], name + ':' + str(uid) + ' was banned.')  # pinylib

    def on_reported(self, reporter):
        self.console_write(tinychat.COLOR['bright_red'], 'You were reported by ' + reporter + '.')  # pinylib
    ...
    def on_quit(self, uid, name):
        if uid is not self.client_id:
            ...
            # then remove user from the room list like usual.
            user = self.find_user_info(name)
            if user is not None:
            ...
            if name in self.room_users.keys():
                ...
                self.console_write(tinychat.COLOR['cyan'], name + ':' + uid + ' left the room.')  # pinylib
    ...
    def on_media_broadcast_start(self, media_type, video_id, time_point, usr_nick):
        ...
        if self.user_obj.is_mod:
            ...
            self.console_write(tinychat.COLOR['bright_magenta'], usr_nick + ' is playing ' + media_type + ' ' +
                               video_id + ' (' + str(time_point) + ')')  # pinylib
    ...
    def on_media_broadcast_close(self, media_type, usr_nick):
        ...
        if self.user_obj.is_mod:
            ...
            if 'pause' in self.last_played_media:
                # Delete pause time point.
                del self.last_played_media['pause']
            self.console_write(tinychat.COLOR['bright_magenta'], usr_nick + ' closed the ' + media_type)  # pinylib
    ...
    def on_media_broadcast_paused(self, media_type, usr_nick):
        ...
        if self.user_obj.is_mod:
            ...
            ts_now = int(tinychat.time.time() * 1000)  # pinylib
            self.last_played_media['pause'] = ts_now - self.media_start_time

            self.console_write(tinychat.COLOR['bright_magenta'], usr_nick + ' paused the ' + media_type)  # pinylib
    ...
    def on_media_broadcast_play(self, media_type, time_point, usr_nick):
        ...
        if self.user_obj.is_mod:
            ...
            self.console_write(tinychat.COLOR['bright_magenta'], usr_nick + ' resumed the ' + media_type +
                               ' at: ' + self.to_human_time(time_point))  # pinylib
    ...
    def on_media_broadcast_skip(self, media_type, time_point, usr_nick):
        ...
        if self.user_obj.is_mod:
            ...
            self.console_write(tinychat.COLOR['bright_magenta'], usr_nick + ' time searched the ' +
                               media_type + ' at: ' + self.to_human_time(time_point))  # pinylib
    ...
    def spam_prevention(self, msg, msg_sender):
        ...
        # TODO: Temporarily ban all incoming messages in all-caps and over the limit of 40 characters.
    ...
    def message_handler(self, msg_sender, msg):
        ...
        # TODO: Implement once the API has been set up.
        # Trigger CleverBot by acknowledging the bot nickname in the message.
        # if self.client_nick in msg:
        #    msgs = msg.split(self.client_nick + ' ')[1]
        #    threading.Thread(target=self.do_cleverbot, args=(msgs, )).start()
        ...
        if msg.startswith(CONFIG['prefix']):
            ...
            # TODO: Do we need this command anymore?
            # cmd == CONFIG['prefix'] + 'camspam':
            #    self.do_camspam()
            ...
            elif cmd == CONFIG['prefix'] + 'votekick':
                threading.Thread(target=self.do_start_voting, args=(cmd_arg, )).start()

            elif cmd == CONFIG['prefix'] + 'stopvotekick':
                self.do_stop_voting()
            ...
            # TODO: CleverBot API has deprecated. Replace with new API or find another solution.
            ...
            self.console_write(tinychat.COLOR['yellow'], self.user_obj.nick + ':' + cmd + ' ' + cmd_arg)  # pinylib

        else:
            # TODO: Handle polling replies once the polling add-on has been made.
            # Handle polling replies.
            # if self.polling and msg.lower() is 'yes' or msg.lower() is 'no':
            #     threading.Thread(target=self.onePoll, args=(self.user_obj.nick, msg.lower(),)).start()

            # Print chat message to console.
            self.console_write(tinychat.COLOR['green'], self.user_obj.nick + ':' + msg)  # pinylib
    ...
    # TODO: We may not actually need this function anymore.
    # def do_camspam(self):
    #     """ Turn on camera spam prevention. """

    #     if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
    #         CONFIG['camspam_prevention'] = not CONFIG['camspam_prevention']
    #         self.send_bot_msg('*Camera Spam Prevention*: *' + str(CONFIG['camspam_prevention']) + '*',
    #                           self.is_client_mod)
            ...
            if len(on_block) is 0:
                ...
                if user is not None:
                    if user.nick not in self.cam_blocked:
                        ...
                        self.send_bot_msg(
                            special_unicode['check_mark'] + ' *' + str(user.nick) + '* is now cam blocked.',
                            self.is_client_mod)
                    else:
                        self.cam_blocked.remove(user.nick)
                        self.send_bot_msg(
                            special_unicode['cross_mark'] + ' *' + str(user.nick) + '* is no longer cam blocked.',
                            self.is_client_mod)
                else:
                ...
                if not bot_user.is_owner or not bot_user.is_mod:
                    if bot_user is not None:
                        ...
                        if bot_user.user_account and bot_user.user_account not in self.botteraccounts:
                            ...
                            tinychat.fh.file_writer(CONFIG['path'], CONFIG['botteraccounts'],
                            ...
                            if bot_user.user_account:
                                for x in range(len(self.botteraccounts)):
                                    if self.botteraccounts[x] == bot_user.user_account:
                                        ...
                                        tinychat.fh.remove_from_file(CONFIG['path'], CONFIG['botteraccounts'], str(bot_user.nick))
                                        break
                            else:
            ...
            if len(new_autoforgive) is not 0:
                ...
                if autoforgive_user is not None:
                    if autoforgive_user.user_account and autoforgive_user.user_account not in self.autoforgive:
                        ...
                        tinychat.fh.file_writer(CONFIG['path'], CONFIG['autoforgive'], str(autoforgive_user.user_account))
                        ...
                        for x in range(len(self.autoforgive)):
                            if self.autoforgive[x] == autoforgive_user.user_account:
                                ...
                                tinychat.fh.remove_from_file(CONFIG['path'], CONFIG['autoforgive'], str(autoforgive_user.user_account))
                                self.send_bot_msg(special_unicode['white_heart'] + ' *' + special_unicode['no_width'] + new_autoforgive +
                                                 special_unicode['no_width'] + '* is no longer protected.', self.is_client_mod)
                                break
                else:
    ...
    def do_clear(self):
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            if self.is_client_mod:
                ...
                self._send_command('privmsg', [clear, tinychat.random_color() + ',en'])  # pinylib
    ...
    # TODO: Integrate back in once vote-kick add-on has been setup properly.
    # def do_start_voting(self, to_kick):
    #     """
    #     Start a vote-kick session.
    #     :param to_kick: str the nickname of the user to start a vote-kick against.
    #     """

    #     if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
    #         if len(to_kick) is 0:
    #             self.send_bot_msg(special_unicode['indicate'] +
    #                                ' Please enter a nickname to start a vote-kick.', self.is_client_mod)
    #         else:
    #             user = self.find_user_info(to_kick)
    #             if user is not None:
    #                 if user.is_owner or user.is_mod:
    #                     self.send_bot_msg(special_unicode['indicate'] +
    #                                        ' You cannot start a vote-kick against this user.', self.is_client_mod)
    #                 else:
    #                     self.startPoll(to_kick)
    #                     threading.Thread(target=self.statusPoll).start()

    # def do_stop_voting(self):
    #     """ Stop a vote-kick session that is in progress. """

    #     if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
    #         if self.polling:
    #             self.polling = False
    #             self.pollers = {}
    #             self.send_bot_msg('The court has been *adjourned*.', self.is_client_mod)
    #         else:
    #             self.send_bot_msg('There is no vote-kick in progress.', self.is_client_mod)
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            if self.is_client_mod:
                if len(bad_nick) is 0:
                    ...
                    badnicks = tinychat.fh.file_reader(CONFIG['path'], CONFIG['badnicks'])  # pinylib
                    if badnicks is None:
                        tinychat.fh.file_writer(CONFIG['path'], CONFIG['badnicks'], bad_nick)  # pinylib
                    else:
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            if self.is_client_mod:
                if len(bad_nick) is 0:
                    ...
                    rem = tinychat.fh.remove_from_file(CONFIG['path'], CONFIG['badnicks'], bad_nick) # pinylib
                    if rem:
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            if self.is_client_mod:
                if len(bad_string) is 0:
                    ...
                    bad_strings = tinychat.fh.file_reader(CONFIG['path'], CONFIG['badstrings'])  # pinylib
                    if bad_strings is None:
                        tinychat.fh.file_writer(CONFIG['path'], CONFIG['badstrings'], bad_string)  # pinylib
                    else:
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            if self.is_client_mod:
                if len(bad_string) is 0:
                    ...
                    rem = tinychat.fh.remove_from_file(CONFIG['path'], CONFIG['badstrings'], bad_string)  # pinylib
                    if rem:
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            if self.is_client_mod:
                if len(bad_account_name) is 0:
                    ...
                    bad_accounts = tinychat.fh.file_reader(CONFIG['path'], CONFIG['badaccounts'])  # pinylib
                    if bad_accounts is None:
                        tinychat.fh.file_writer(CONFIG['path'], CONFIG['badaccounts'], bad_account_name)  # pinylib
                    else:
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            if self.is_client_mod:
                if len(bad_account) is 0:
                    ...
                    rem = tinychat.fh.remove_from_file(CONFIG['path'], CONFIG['badaccounts'], bad_account)  # pinylib
                    if rem:
            ...
            if self.is_client_mod:
                if len(list_type) is 0:
                    ...
                    if list_type.lower() == 'bn':
                        bad_nicks = tinychat.fh.file_reader(CONFIG['path'], CONFIG['badnicks'])  # pinylib
            ...
            if self.is_client_mod:
                if len(user_name) is 0:
                    ...
                    if user is None:
                        ...
                        self.send_owner_run_msg('*User Account Type:* ' + str(user.user_account_type))
                        self.send_owner_run_msg('*User Account Gift Points:* ' + str(user.user_account_giftpoints))
                        self.send_owner_run_msg('*Device Type:* ' + str(user.device_type))
                        # self.send_owner_run_msg('*Broadcast Monitor:* ' + str(user.is_playable))
    ...
    def do_close_media(self):
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            if self.media_timer_thread is not None and self.media_timer_thread.is_alive():
                ...
                self.console_write(tinychat.COLOR['bright_magenta'], 'Closed the ' + self.last_played_media['type']) # pinylib
    ...
    def do_pause_media(self):
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            ...
            self.console_write(tinychat.COLOR['bright_magenta'], 'Paused the ' +
    ...
    def do_resume_media(self):
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            if 'pause' in self.last_played_media:
                ...
                self.media_event_timer(new_media_time)
                self.console_write(tinychat.COLOR['bright_magenta'], 'Resumed the ' +
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            ...
            if len(new_time_point) is 0:
                ...
                if self.last_played_media['video_time'] <= time_point:
                    ...
                    self.media_event_timer(new_media_time)
                    self.console_write(tinychat.COLOR['bright_magenta'], 'Time searched the ' +
                                       self.last_played_media['type'] + ' at: ' + self.to_human_time(time_point))

    # == Public Command Methods. ==
    def do_version(self):
        """ Replies with relevant version information concerning the bot. """
        self.send_undercover_msg(self.user_obj.nick, '*pinybotAlpha* ' + str(__version__))
        self.send_undercover_msg(self.user_obj.nick, '*Authors:* ' + str(authors))
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            if not self.syncing:
                if len(self.playlist) is not 0:
                    ...
                    tinychat.time.sleep(0.9)  # pinylib
                    self.syncing = False
                else:
            ...
            if len(playlist_search) is 0:
                ...
                if self.play_lists is None:
                    ...
                    for x in range(len(self.play_lists)):
                        ...
                        tinychat.time.sleep(0.2)  # pinylib
    ...
    def do_spy(self, room_name):
        ...
        if self.is_client_mod:
            if len(room_name) is 0:
                ...
                spy_info = tinychat.tinychat_api.spy_info(room_name)  # pinylib
                if spy_info is None:
    ...
    def do_account_spy(self, account):
        ...
        if self.is_client_mod:
            if len(account) is 0:
                ...
                tc_usr = tinychat.tinychat_api.tinychat_user_info(account)  # pinylib
                if tc_usr is None:
    ...
    # TODO: Needs a method to close the active connection.
    # def do_cleverbot(self, qs):
    #     """
    #     Clever Bot API connection to handle statements/queries. Producing a response similar to an A.I.
    #     :param qs: str a question/statement to send to the CleverBot API.
    #     """

    #     if len(qs) is not 0:
    #         if self.cleverbot_client is not None:
    #             # Wield the existing connection to query Cleverbot.
    #             answer = self.cleverbot_client.ask(qs)
    #             self.send_bot_msg('*' + str(self.client_nick) + '*: ' + str(answer), self.is_client_mod)
    #         else:
    #             self.send_bot_msg(special_unicode['state'] + ' *Waking* up ' + self.client_nick + '.', self.is_client_mod)
    #             # Establish a connection if one was not already present.
    #             self.cleverbot_client = cleverbot.Cleverbot()
    #             # Retry the user's query with an active CleverBot connection.
    #             self.do_cleverbot(qs)
    #     else:
    #         self.send_bot_msg(special_unicode['indicate'] + ' You are talking to an A.I. Please enter a question/statement to converse.',
    #                           self.is_client_mod)
        ...
        if 'http' in msg or 'https' in msg:
            if '!' not in msg and 'tinychat.com' not in msg:
                ...
                if url is not None:
                    ...
                    self.console_write(tinychat.COLOR['cyan'], self.user_obj.nick + ' posted a URL: ' + url)

    # TODO: Integrate with akinator module.
    # def do_akinator_start(self):
    #     """ Start a new Akinator session. """

    #     if not self.akin_session[0]:
    #         self.Akinator(str(self.user_obj.nick))
    #     else:
    #         self.send_private_bot_msg('An Akinator session is already in progress. Please wait until it finishes.', self.user_obj.nick)

    # def do_stop_akinator(self, force_stop=False):
    #     """
    #     Stops an active Akinator session if the user who started it requests so.
    #     :param force_stop: bool True/False if you want to forcibly reset an Akinator session.
    #     """

    #     if not force_stop:
    #         if self.user_obj.nick == self.akin_session[1]:
    #             self.akin_session = [False, '']
    #             self.akin_answer = None
    #             self.send_private_bot_msg('The current *Akinator* session was stopped.', self.user_obj.nick)
    #     else:
    #         self.akin_session = [False, '']
    #         self.akin_answer = None
    ...
    # TODO: Handle invalid tags provided and list all available tags.
    def do_one_liner(self, tag):
        ...
        if tag:
            if tag == '?':
                ...
                self.send_bot_msg('The tag you specified is not available. Enter *!joke ?* to get a list of tags.', self.is_client_mod)
        else:
    ...
    def private_message_handler(self, msg_sender, private_msg):
        ...
        self.check_msg_for_bad_string(private_msg, pm=True)

        # TODO: Place back in when "Akinator" has been integrated as a module for the bot.
        # See if an "Akinator" session is in progress and if the reply is from the guessing user, then count it
        # if self.akin_session[0]:
        #     if str(msg_sender) == self.akin_session[1]:
        #         # TODO: Just check the range function between various ranges (we want all numbers between 0-5 only),
        #         #       or go back to using ['0', '1', .., '5'].
        #         if int(private_msg.strip()) in range(0, 6):  # TODO: Check if this line actually works or not.
        #             self.akin_answer = str(private_msg)
        ...
        if private_msg.startswith(CONFIG['prefix']):
            ...
            # TODO: Stream logo (image file), GIF or video open and close; all media types.
            #       are formatted as an FLV file which can be iterated over.
            # elif pm_cmd == CONFIG['prefix'] + 'logo':
            # abstract: do_logo_stream (args)

            # elif pm_cmd == CONFIG['prefix'] + 'gif':
            #    abstract: do_gif_stream (args)

            # elif pm_cmd == CONFIG['prefix'] + 'video':
            # abstract: do_video_stream (args)

            # elif pm_cmd == CONFIG['prefix'] + 'stopvideo':
            # abstract: do_stop_video_stream (args)
            ...
            # TODO: Command to be enabled once the module has been created as an add-on.
            # elif pm_cmd == CONFIG['prefix'] + 'akinator':
            #    threading.Thread(target=self.do_akinator_start).start()

            # elif pm_cmd == CONFIG['prefix'] + 'stopakinator':
            #    self.do_stop_akinator()

        # Print to console.
        self.console_write(tinychat.COLOR['white'], 'Private message from ' + self.user_obj.nick + ': ' +
    ...
    def do_clear_bad_nicks(self):
        ...
        if self.user_obj.is_owner:
            tinychat.fh.delete_file_content(CONFIG['path'], CONFIG['badnicks'])  # pinylib

    def do_clear_bad_strings(self):
        """ Clears the bad strings file. """
        if self.user_obj.is_owner:
            tinychat.fh.delete_file_content(CONFIG['path'], CONFIG['badstrings'])  # pinylib

    def do_clear_bad_accounts(self):
        """ Clears the bad accounts file. """
        if self.user_obj.is_owner:
            tinychat.fh.delete_file_content(CONFIG['path'], CONFIG['badaccounts'])  # pinylib
        ...
        if self.user_obj.is_owner or self.user_obj.is_mod or self.user_obj.has_power:
            if len(key) is 0:
                ...
                if key == self.key:
                    ...
                    self.console_write(tinychat.COLOR['red'], 'Disconnected by ' + self.user_obj.nick + '.')  # pinylib
                    self.disconnect()
                else:
    ...
    # TODO: Construct the functions to stream.
    # abstract: procedure do_logo_stream

    # abstract: procedure do_gif_stream

    # abstract: procedure do_video_stream

    # abstract: procedure do_stop_video_stream
    ...
    def media_event_handler(self):
        ...
        if len(self.playlist) is not 0:
            if self.inowplay >= len(self.playlist):
                ...
                tinychat.time.sleep(2.5)  # pinylib
                if self.is_connected:
                    self.last_played_media = self.playlist[self.inowplay]
                    self.send_media_broadcast_start(self.playlist[self.inowplay]['type'],
                                                    self.playlist[self.inowplay]['video_id'])
    ...
    def media_event_timer(self, video_time):
        ...
        self.media_start_time = int(tinychat.time.time() * 1000)  # pinylib
    ...
    # Streaming function.
    # TODO: Move this method to elsewhere in the file
    def stream_process(self, media_path, media_file):
        """
        Setup and start a broadcasting stream with the bot.
        :param media_path: str the path to the directory of the file.
        :param media_file: str the name of the file.
        """
        # Configure client publishing
        self._set_upstream()
        tinychat.time.sleep(5)
        # Start sending media frames:
        # Load the frame data from the FLV file
        frames = self._load_media(media_path, media_file)
        # Send iterated frames
        self._send_frames(frames)
        # Finalize and close the stream
        self._send_close_stream()
        # Request for the stream to be deleted on the server
        self._send_delete_stream()
        # Delete our local record of the open stream
        self.tidy_streams(self.streams['client_publish'])
    ...
    # TODO: Decide whether the config/log structure should be as such.
    ...
    def get_uptime(self):
        ...
        """
        up = int(tinychat.time.time() - self.init_time)  # pinylib
        return up * 1000

    def format_time_point(self, raw_time_point):
        """
        ...
        # Total milliseconds
        milliseconds = int(time_point_components[0]) * hour + int(time_point_components[1]) * minute + int(
            time_point_components[2]) * second
        return milliseconds
...
def main():
    if CONFIG['auto_connect']:
        ...
        room_password = tinychat.getpass.getpass('Enter room password (optional:password hidden): ')
        nickname = 'pinybotAlpha'  # raw_input('Enter nick name (optional): ')
        login_account = raw_input('Login account (optional): ')
        login_password = tinychat.getpass.getpass('Login password (optional:password hidden): ')
    ...
    if client.is_connected:
        ...
        a.daemon = True
        a.start()
        client.console_write(tinychat.COLOR['white'], 'Started alive management.')
        ...
        p.daemon = True
        p.start()
        client.console_write(tinychat.COLOR['white'], 'Started ping management.')
    ...
    if CONFIG['webserver']:
        ...
        while client.is_connected:
            ...
            # EXPERIMENTAL AREA
            elif chat_msg.lower() == '/upstream':

                # Load file
                media_path = 'files/media_out/'
                media_file = raw_input('\nMedia file:')  # Full media file location (with extension)
                print('Media supposedly at: ' + media_path + media_file)
                if os.path.exists(media_path + media_file):
                    print('File found.')
                else:
                    print('File not found.')
                    continue

                    # tag_ask = raw_input('Saved as tag instead? (y/n)')
                    # if tag_ask.lower() == 'y':
                    # pass
                    # else:
                    # continue

                threading.Thread(target=client.stream_process, args=(media_path, media_file )).start()

            # Issue a play command into the stream
            # elif chat_msg.lower() == '/downstream':
            #  nick = raw_input('Nick name: ')
            #  if nick in client.room_users:
            #      playID = client.find_user_info(nick).id
            #  else:
            #      print 'Invalid nickname.'
            #      continue

            #  client._send_create_stream()
            #  while client.stream_sort:
            #      tinychat.time.sleep(1)  # pinylib
            #  client._send_play(client.create_stream_id, playID)
                ...
                if CONFIG['console_msg_notice']:
                    ...
                    client.console_write(tinychat.COLOR['cyan'], 'You:' + chat_msg)
