...
class TinychatRTMPClient:
    ...
    def _callback(self):
        ...
        while self.is_connected:
            try:
                ...
                failures = 0
            try:
                # Handle 'reverse' ping/pong requests.
                if amf0_data_type == rtmp_protocol.data_types.USER_CONTROL \
                        and amf0_data['event_type'] == rtmp_protocol.user_control_types.PING_RESPONSE:
    ...
    def on_join(self, join_info_dict):
        ...
        # if user.is_playable:
        #   self.play_publishers[user.nick] = user.id
    ...
    def on_joinsdone(self):
        ...
        # self.play_connection = True
        # threading.Thread(target=self.start_play_connect).start()
    ...
    def _send_command(self, cmd, params=[]):
        ...
        NOTE: The transaction ID may have no effect on the command messages.
    ...
    def _load_media(self, media_path, media_file):
        """
        Load and encode media files (flv, mp4 etc.) or image files (jpg, gif etc.), then stream them to the RTMP Server.
        :param media_path:
        :param media_file: str name of the file (with extension).
        """
        # :param frame_delay: int/float the delay between sending frames (0.02 for 10 FPS default).

        ############################################################################################
        # Tag Loader/Saver
        # Load pre-encoded .tdata file (containing the FLV data) if it exists or generate new one
        # encoded_tdata_file = CONFIG['tdata_path'] + 'encoded_' + file_title + '.tdata'
        # tag_content = tag_handler.load_frame_data(encoded_tdata_file)
        # if tag_content is not None:
        # generated_frames = tag_content
        # else:
        ############################################################################################

        # Setup media names and locations
        media_location = media_path + media_file
        print('Debug: location' + media_location + ' path: ' + media_path + ' name of file: ' + media_file)

        if os.path.exists(media_location):

            # Normal encoder
            # self.console_write(COLOR['white'], 'Encoding media file, ' + media_file + ', to flv.')
            # encoded_file = 'encoded_' + media_file.split('.')[0] + '.flv'
            # print('Encoded file: ' + encoded_file)
            #  FFMPEG.main(1, media_location, encoded_file)

            # Iterate over the newly encoded FLV file
            self.console_write(COLOR['white'], 'Retrieving tag/frame data.')
            generated_frames = tag_handler.iterate_frames(media_location)  #encoded_file

            # Store information in ".tdata" file simultaneously
            # threading.Thread(target=tag_handler.store_frame_data, args=(encoded_file, generated_frames, )).start()
            # Send the appropriate A/V packets
    ...
    def _send_frames(self, frames):
        ...
        if frames is not None:
            for x in xrange(len(frames)):
                ...
                self._configure_av_packet(frames[x])

                # TODO: Need a better implementation of frame delay.
                # Apply frame delay; 10 FPS FLV ~ 0.02
                if not self.play_audio or not self.play_video:
                    time.sleep(0.025)
                else:
                    time.sleep(0.02)

            # Acknowledge locally that we have finished streaming
            self.console_write(COLOR['white'], 'Streaming complete.')
        else:
            self.console_write(COLOR['white'], 'No frames were received. Exiting streaming.')
    ...
    #     """
    #     Allows for the publish of a small FLV file, flv1 codec, with an encoded image.
    #     :param logo_flv_file: str the name of the FLV file containing the image.
    #     """
    #     # Parse the data within the FLV file
    #     logo_frames = self._load_media(logo_flv_file)

    # Send video packet
    #     self._send_iterated_frames(logo_frames, image=True)
    # --------------------------------------------------------------------------
